...
Phase 0 sets up Bun workspaces + Nx without changing any existing build, deploy, or release behavior.

### 0.1 Enable Bun Workspaces
...
**`bun.lock`** — committed at repo root as the workspace lockfile.
...
> [!TIP]
> Nx infers tasks from each package's `package.json` scripts — no Nx executors or `project.json` files are needed initially. Running `npx nx build @gv-tech/design-system` will call the existing `bun run build` script.
...
```yaml
# After bun install step, add:
- name: Restore Nx Cache
  uses: actions/cache@v4
  with:
    path: .nx/cache
    key: ${{ runner.os }}-nx-${{ hashFiles('bun.lock') }}-${{ github.sha }}
    restore-keys: |
      ${{ runner.os }}-nx-${{ hashFiles('bun.lock') }}-
      ${{ runner.os }}-nx-
```

Existing `bun run lint`, `bun run test:ci`, `bun run build`, and `bun run build:site` scripts continue to run as-is. No Nx commands in CI yet — that comes when packages have their own scripts.

### 0.5 Deployment — No Changes

| Setting             | Current Value                          | Phase 0 Change |
| :------------------ | :------------------------------------- | :------------- |
| Build command       | `bun run build:site`                   | None           |
...
- [x] Decided: skip `"private": true` for now (Option B — Bun supports workspaces without requiring it)
- [x] `bun add -d nx @nx/js` (Nx 22.5.1)
...
- [x] Verify: `bun install` ✅
- [x] Verify: `bun run build` ✅ (library → `dist/`)
- [x] Verify: `bun run build:site` ✅ (site → `dist-site/`)
- [x] Verify: `bun run test:ci` ✅ (120 tests, 49 files)
...
The root `package.json` currently publishes as `@gv-tech/design-system`. Adding `"private": true` (not required for Bun workspaces root) would prevent npm publishing from root.
...
| **B) Skip `private:true`, use workspaces anyway** | Bun allows workspaces without `private:true` at root. Publishing continues from root. | ✅ **Recommended for Phase 0**              |
...
### Decision 2: Convert to Bun Workspaces Now or Later

| Option                   | Pros                                                                                                                          | Cons                                                                                    |
| :----------------------- | :---------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------- |
| **Now (Phase 0)**        | New packages can immediately cross-reference each other; `bun install` resolves them. Clean foundation before any code moves. | Slightly more work upfront in the "no-op" PR.                                           |
...
| :---------------------------------------- | :--------- | :----- | :---------------------------------------------------- |
| Bun workspaces breaks `bun install`       | Low        | High   | Test in CI on the Phase 0 PR before merging           |
...
- [Nx + Bun Workspaces](https://nx.dev/recipes/adopting-nx/adding-to-monorepo) — Official Nx integration docs
